...
function poll_dev($dev) {
  ...
  if(file_exists($dev)) {
    ...
    $timeout = microtime(true) + 1.0; // 1 sec timeout. 
    ...
    fwrite($fp,"s",1);  // Send a Save data command
    fflush($fp);    
    ...
    do {
      ...
      do {                // Do reads until data arrive.
        ...
        $resp .= $c;
        if(ord($c) == 10) $done=true;
      }   while(! $done && microtime(true) < $timeout);

      if($done) {
         //echo "Resp: " . $resp;
         $done  = process_resp($resp);
      }

      if(!$done) {
	fwrite($fp,"r",1);  // Send a Recall data command
        fflush($fp);
        $resp = "";         // start over.    
      } 
...
function process_resp($resp) {
  ...
  if(count($out) != 20) {
    echo count($out) . "/ Fields.  Response:\n" . $resp . "\n";
    return false;
  } else {
    $last_comma = strrpos($resp,",");
    $buf = substr($resp,0,$last_comma+1); // copy only up through the last comma.

    $crc = crc16_compute($buf);
    if($crc != $out[19] )  {
      echo "CRC: " . $crc .  " " . $out[19] . " buf: " . $buf . "\n";
      return false;
    }
    // Extract the group ID and do some sanity checking.    
    $group = hexdec($out[0]);
    $tdiff = (intval($out[2])/1000.0) - (intval($out[1])/1000.0); // seconds difference
...
//////////////////////////////
// return the CRC-16 for the buffer.
// Reimplementation of IBM CRC-16

function crc16_compute($buf)
{
  $crc = 0;
  $len = strlen($buf);

  for($i = 0; $i < $len; $i++ ) {
    $crc = ($crc ^ ord($buf[$i])) & 0xFFFF;
    for ($j = 0; $j < 8; $j++) {
      if ($crc & 1) $crc = (($crc >> 1) ^ 0xA001) & 0xFFFF;
...
class GroupRec {
  ...
  function save() {
    ...
    $path = sprintf("%s/G%02d_%s",$folder,$this->GroupNum,$csv_fname);
    if(!file_exists($path)) {
	$fp = fopen($path,"c");
  ...
  function update_odometer_file($path) {
	...
	$odo_arr = array();
        ...
        $outstr = sprintf("%d,%s,%.5f,  ",$this->GroupNum,$tstring,$this->Tdiff/3600.0);
	...
	for($i=0; $i < 5; $i++)  {
            ...
            $outstr = sprintf("%.1f,%.2f,%.5f",$this->CageArr[$i]->dist_m,$active_vel,$hours_active);
            fputs($fp,$outstr);
            if($i < 4) {
               fputs($fp,"  ,");
            } else {
	...
	$odo_arr = fgetcsv($fp,256);   // Read the header values
	$odo_arr = fgetcsv($fp,256);   // Read the existing values
	...
	rewind($fp);   // Reset the file pointer
        ...
        $outstr = sprintf("%d,%s,%.5f,  ",$this->GroupNum,$tstring,$trip_time);
	fputs($fp,$outstr);  // Output GroupID , Time, Elaped Time in hours
	...
	for($i=0; $i < 5; $i++)  {
            ...
            $outstr = sprintf("%.1f,%.2f,%.5f",$tot_dist,$active_vel,$hours_active);
            fputs($fp,$outstr);
            if($i < 4) {
               fputs($fp,"  ,");
            } else {
...
Class CageRec {
   ...
   public function __construct($msg_arr,$group_cfg,$cur_index) {
      ...
      $tdiff = (intval($msg_arr[2]) - intval($msg_arr[1])) / 1000.0; // Convert to seconds

      $indx = $cur_index + 4; // Skip over GID, T1, T2, slot
